...
function normalizedFrame(frame: Record<string, unknown>): Record<string, unknown> {
  ...
  return Array.isArray(normalized) ? (normalized[0] as Record<string, unknown>) : frame;
}
...
export function MotionRuntime({
  ...
  useEffect(() => {
    ...
    const addFallbackInteractions = () => {
      const menuTriggers = Array.from(
        document.querySelectorAll<HTMLElement>("[data-framecoded-trigger='hycgpg9']"),
      )
        .map((trigger) => trigger.closest<HTMLElement>("[data-border='true']") ?? trigger)
        .filter((trigger, index, all) => !trigger.closest("[aria-expanded]") && all.indexOf(trigger) === index);
      ...
      const openMenu = () => {
        if (overlay) return;
        const nextOverlay = document.createElement("nav");
        ...
        nextOverlay.querySelectorAll("a").forEach((link) => link.addEventListener("click", closeMenus));
        document.body.appendChild(nextOverlay);
        overlay = nextOverlay;
      };
      const onMenuClick = (event: MouseEvent) => {
        event.preventDefault();
        if (overlay) closeMenus();
        else openMenu();
      };
      ...
      menuTriggers.forEach((trigger) => {
        ...
        trigger.addEventListener("click", onMenuClick);
        trigger.addEventListener("keydown", onMenuKeyDown);
      });
      document.addEventListener("keydown", onEscape);
      listenerCleanups.push(() => {
        menuTriggers.forEach((trigger) => {
          trigger.removeEventListener("click", onMenuClick);
      ...
      const forms = Array.from(document.querySelectorAll<HTMLFormElement>("form:not([action])"));
      const onFormSubmit = (event: SubmitEvent) => {
        event.preventDefault();
        const form = event.currentTarget as HTMLFormElement;
        const values = [...new FormData(form).entries()]
          .filter(([name]) => !["website", "company"].includes(name))
          .map(([name, value]) => `${name}: ${String(value)}`)
          .join("\n");
        window.location.href = `mailto:info@aasco.co.in?subject=${encodeURIComponent("Aasco Reality website enquiry")}&body=${encodeURIComponent(values)}`;
      };
      forms.forEach((form) => form.addEventListener("submit", onFormSubmit));
      listenerCleanups.push(() => forms.forEach((form) => form.removeEventListener("submit", onFormSubmit)));
    };
        ...
        for (const { motion, index: motionIndex } of activeMotions) {
          ...
          if (bootstrapEntries) {
            for (let index = bootstrapEntries.length - 1; index >= 0; index--) {
              ...
              if (entry.id !== id || entry.index !== motionIndex || entry.element !== element)
                continue;
              ...
              for (const element of elements) {
                ...
                if (sampledKeyframes?.length) {
                  ...
                  const sampledAnimation = element.animate(waapiKeyframes(sampledKeyframes), {
                    duration: reduced
                      ? 0
                      : Number(open ? target.openDuration : target.closeDuration) ||
    ...
    if (cursors.length && !window.matchMedia("(pointer: coarse)").matches) {
        ...
        try {
          ...
          if (Array.isArray(parsed) && parsed.length > 1) {
            opacityKeyframes = parsed
              .filter(
                (frame) => Number.isFinite(Number(frame.offset)) && frame.opacity !== undefined,
              )
      ...
      const place = (cursor: HTMLElement, x: number, y: number) => {
        cursor.style.transform = "translate(-50%, -50%) translate3d(" + x + "px," + y + "px,0)";
      };
      const show = (entry: (typeof cursorEntries)[number], event: PointerEvent) => {
        ...
        if (reduced || entry.duration <= 0 || !entry.opacityKeyframes?.length) {
          ...
          entry.fade = entry.cursor.animate(entry.opacityKeyframes, {
      ...
      listenerCleanups.push(() => {
        ...
        for (const entry of cursorEntries) {
          entry.fade?.cancel();
          if (entry.trigger && entry.enter)
            entry.trigger.removeEventListener("pointerenter", entry.enter);
          if (entry.trigger && entry.leave)
            entry.trigger.removeEventListener("pointerleave", entry.leave);
